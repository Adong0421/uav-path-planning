...
def adjust_sweep_width_for_subregion(directory, initial_sweep_width):
    """遍历每个子区域，并动态调整扫描宽度，直到满足条件为止。返回每个子区域的扫描路径和最终的扫描宽度。"""
    final_sweep_widths = {}
    scan_areas = {}
    subregion_grid_counts = {}  # 用于记录每个子区域的栅格数量
    for filename in os.listdir(directory):
        if filename.endswith(".npy"):
            ...
            # 记录子区域的栅格数量
            subregion_grid_counts[filename] = np.sum(scan_area)
    ...
    return scan_areas, final_sweep_widths, subregion_grid_counts
...
if __name__ == "__main__":
    directory = "subregions"
    subpath_directory = 'subpaths'
    grid = load_grid('grid.npy')
    center = (1200, 1200)  # 假定的区域中心点，根据需要调整
    initial_sweep_width = 36  # 扫描宽度
    size = 750
    fov_angle = 80  # 视场角度为80度
    grid_scale = 1  # 栅格比例为1
    scan_areas, final_sweep_widths, subregion_grid_counts = adjust_sweep_width_for_subregion(directory, initial_sweep_width)
    # 打印每个子区域的栅格数量
    print("每个子区域的栅格数量：")
    for filename, grid_count in subregion_grid_counts.items():
        print(f"{filename}: {grid_count}")